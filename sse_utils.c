...
/*
 * sse_utils_vdivs: do vector point-wise division on single-precision
 * (32-bit) floats
 *
 * equivalent to "for (i = 0; i < len; i++) c[i] = a[i] / b[i];"
 */
void sse_utils_vdivs(float *c, const float *a, const float *b, int len)
{
	register int i;
	register int vlen;
	__m256 c_256, a_256, b_256;
	...
	for (i = 0; i <= vlen; i += 8) {
		...
		c_256 = _mm256_div_ps(a_256, b_256);
...
/*
 * sse_utils_vdivd: do vector point-wise division on double-precision
 * (64-bit) floats
 *
 * equivalent to "for (i = 0; i < len; i++) c[i] = a[i] / b[i];"
 */
void sse_utils_vdivd(double *c, const double *a, const double *b, int
		len)
{
	register int i;
	register int vlen;
	...
	for (i = 0; i <= vlen; i += 4) {
		...
		c_256 = _mm256_div_pd(a_256, b_256);
